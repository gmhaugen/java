...
/*
 * @Author: 140464
 * @Description: Class for enciphering cleartext using the Beaufort cipher.
 * Input can only be uppercase.
 * 
 */

public class BeaufortEncoder {

	// Holds the 26 letters of the english alphabet and the corresponding
	// numbers
	private Map<Character, Integer> lettersToNumbers;

	// Holds the numbers 0-25 and the corresponding letters of the english
	// alphabet
	...
	public void encode(String clearText, String key) {
		...
		for (int i = 0; i < clearTextNumbers.size(); i++) {
			...
			// Making sure that we restart the iteration of the key if we are at
			// the last letter
			...
			encodedValue = keyNumbers.get(keyCounter) - clearTextNumbers.get(i);
			
			// Because of a bug that can make the encoded value negative,
			// this line needs to be in place.
			while (encodedValue < 25) {
				encodedValue += 26;
			}
			
			// Here we do the mod26 operation to make sure the calculated value
			// is not higher than 25
			// (the numeric representation of the last letter in the english
			// alphabet)
			while (encodedValue > 25) {
				encodedValue -= 26;
			}
			...
			// Adding the encoded value (and turning numeric to character) to
			// the stringbuilder
	...
	public void printAll() {
		...
		for (int i = 0; i < cipherTextNumbers.size(); i++) {
			if (cipherTextNumbers.get(i) != null)
				System.out.print(cipherTextNumbers.get(i));
			
			if (i != clearTextNumbers.size() - 1 && cipherTextNumbers.get(i) != null)
				System.out.print("-");
		}
		System.out.print(")");
